...
>
	...
	<div class="pointer-events-none absolute inset-0 overflow-hidden">
		<div
			class="absolute -top-40 -right-40 h-96 w-96 animate-pulse rounded-full bg-blue-600/20 blur-3xl"
		></div>
		<div
			class="absolute -bottom-40 -left-40 h-96 w-96 animate-pulse rounded-full bg-cyan-600/20 blur-3xl"
		></div>
	</div>
	...
	<div class="relative z-10 w-full max-w-md">
			...
			<div class="p-8 sm:p-10">
				...
				>
					...
					<div class="grid grid-cols-2 gap-3">
						<div class="space-y-2">
							<label for="firstName" class="text-sm font-semibold text-slate-300"
								>{$t('register.firstNameLabel')}</label
							>
							<input
								id="firstName"
								name="firstName"
						...
						<div class="space-y-2">
							<label for="lastName" class="text-sm font-semibold text-slate-300"
								>{$t('register.lastNameLabel')}</label
							>
							<input
								id="lastName"
								name="lastName"
					...
					<div class="space-y-2">
						...
						<div class="relative">
							...
							<input
								...
								class="w-full rounded-xl border border-slate-700 bg-slate-800/50 py-3.5 pr-4 pl-12 text-white placeholder-slate-500 transition-all outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
					...
					<!-- Confirm Email -->
					<div class="space-y-2">
						<label for="confirmEmail" class="text-sm font-semibold text-slate-300"
							>{$t('register.confirmEmailLabel')}</label
						>
						<input
							id="confirmEmail"
							name="confirmEmail"
							...
							class="w-full rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3.5 text-white placeholder-slate-500 transition-all outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 {confirmEmail &&
							email !== confirmEmail
								? 'border-red-500/50'
								: ''}"
					...
					<!-- Password -->
					<div class="space-y-2">
						<label for="password" class="text-sm font-semibold text-slate-300"
						...
						<div class="relative">
							...
							<input
								...
								bind:value={password}
								class="w-full rounded-xl border border-slate-700 bg-slate-800/50 py-3.5 pr-12 pl-12 text-white placeholder-slate-500 transition-all outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
						...
						{#if password}
							<div class="flex items-center gap-2">
								<div class="h-1 flex-1 rounded-full bg-slate-700">
									<div
										class="h-1 rounded-full transition-all {strengthColor}"
					...
					<!-- Confirm Password -->
					<div class="space-y-2">
						<label for="confirmPassword" class="text-sm font-semibold text-slate-300"
					...
					>
						<div
							class="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent transition-transform group-hover:translate-x-full group-hover:duration-1000"
						></div>
				...
				<div class="my-8 flex items-center gap-4">
					...
					<span class="text-xs text-slate-500 uppercase">{$t('register.divider')}</span>
					<div class="h-px flex-1 bg-slate-700"></div>
				</div>
				...
				<div class="flex justify-center">
					<button
						...
						class="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3 text-sm font-semibold text-white transition-all hover:border-slate-600 hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
					>
						{#if isGoogleSubmitting}
							<Spinner size="sm" color="text-slate-300" />
							{$t('register.submitting')}
						{:else}
		...
		<a
			href="/"
			class="mt-6 flex items-center justify-center gap-2 text-sm text-slate-400 transition-colors hover:text-slate-300"
		>
