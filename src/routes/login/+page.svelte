		...
		>
			<div class="p-8 sm:p-10">
				...
				<form
					...
					use:enhance={() => {
						if (!email || !password) {
							errorMessage = $t('login.errorEmpty');
							return ({ update }) => {
								update({ reset: false });
							};
						}
				...
				<div class="my-8 flex items-center gap-4">
					...
					<span class="text-xs text-slate-500 uppercase">{$t('login.divider')}</span>
					<div class="h-px flex-1 bg-slate-700"></div>
				</div>
...
{#if showForgotPasswordModal}
	...
	>
		<div
			class="relative w-full max-w-md overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/95 shadow-2xl"
		>
			<div class="p-8">
				<!-- Header -->
				<div class="mb-6 text-center">
					...
					<h3 class="mb-2 text-2xl font-bold text-white">{$t('login.forgot.title')}</h3>
					...
					>
						<div class="flex items-start gap-2">
							...
							<div>
								...
								<p class="mt-1 text-xs text-green-400/80">{$t('login.forgot.successHint')}</p>
							</div>
						</div>
					</div>
				{/if}
				...
				{#if !resetSuccessMessage}
					<form
						...
						use:enhance={() => {
							if (!forgotEmail) {
								resetErrorMessage = $t('login.forgot.errorEmpty');
								return ({ update }) => {
									update({ reset: false });
								};
							}
							...
							return async ({ result, update }) => {
								...
								if (result.type === 'failure' && result.data) {
									const errorKey = `auth.error.${result.data.error}`;
									resetErrorMessage = $t(errorKey as any);
								} else if (result.type === 'success' && result.data?.resetSent) {
									resetSuccessMessage = $t('auth.success.resetSent');
								}
					...
					>
						<div class="space-y-2">
							<label for="forgotEmail" class="text-sm font-semibold text-slate-300"
								>{$t('login.forgot.emailLabel')}</label
							>
							<div class="relative">
								<div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
									<svg
			...
			<button
				onclick={closeForgotPasswordModal}
				aria-label={$t('login.forgot.closeModal')}
				class="absolute top-4 right-4 rounded-lg p-1 text-slate-400 transition-colors hover:bg-slate-800 hover:text-slate-200"
			>
