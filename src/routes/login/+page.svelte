<script lang="ts">
	...
	async function handleLogin(e: SubmitEvent) {
		...
		errorMessage = $t('login.errorNoBackend');
		isSubmitting = false;
	}
	...
	async function handleForgotPassword(e: SubmitEvent) {
		...
		isSendingReset = true;
		await new Promise((resolve) => setTimeout(resolve, 500));
		resetSuccessMessage = $t('login.forgot.errorDisabled');
		...
		<div class="overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/50 shadow-2xl backdrop-blur-xl">
			<div class="p-8 sm:p-10">
				...
				<form onsubmit={handleLogin} class="space-y-5">
					<!-- Email -->
					<div class="space-y-2">
						<label for="email" class="text-sm font-semibold text-slate-300">{$t('login.emailLabel')}</label>
						<div class="relative">
					...
					<div class="space-y-2">
						<div class="flex items-center justify-between">
							<label for="password" class="text-sm font-semibold text-slate-300">{$t('login.passwordLabel')}</label>
							<button 
								type="button" 
				...
				<div class="my-8 flex items-center gap-4">
					...
					<span class="text-xs uppercase text-slate-500">{$t('login.divider')}</span>
					<div class="h-px flex-1 bg-slate-700"></div>
				</div>
			...
			<div class="p-8">
				...
				{#if !resetSuccessMessage}
					<form onsubmit={handleForgotPassword} class="space-y-4">
						<div class="space-y-2">
							<label for="forgotEmail" class="text-sm font-semibold text-slate-300">{$t('login.forgot.emailLabel')}</label>
							<div class="relative">
								<div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
									<svg class="h-5 w-5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" />
