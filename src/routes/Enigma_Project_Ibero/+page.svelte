...
<script lang="ts">
  ...
  const stepMachine = () => {
    ...
    if (currentState === 'qRead') {
      ...
      currentState = 'qWrite';
      stepCount += 1;
      appendTrace(`Paso ${stepCount}: qDecode aplica codigo ${machineKey} y obtiene '${pendingDecoded}'.`);
...
<main class="layout" aria-labelledby="title">
  <header class="hero">
    ...
    <p class="subtitle">
      Introduce una fecha, genera un codigo diario y observa como la maquina traduce el mensaje encriptado
      hasta recuperar el texto original.
    </p>
  </header>
  ...
  <div class="split-view">
    <div class="work-area">
      <section class="card" aria-labelledby="setup-title">
        ...
        <div class="grid-2">
          <div>
            ...
            <div class="input-row">
              <input id="reference-date" type="date" bind:value={referenceDate} on:change={ensureGeneratedCode} required />
            </div>
          </div>
          <div>
        ...
        <div class="buttons">
          <button id="encrypt-btn" type="button" on:click={prepareEncryption} disabled={running}>Encriptar</button>
      ...
      <section class="card" aria-labelledby="machine-title">
        ...
        <div class="buttons">
          <button id="step-btn" type="button" class="secondary" on:click={stepMachine} disabled={running || halted}
            >Paso</button
          >
          <button id="run-btn" type="button" class="secondary" on:click={runMachine} disabled={running || halted}
            >Ejecutar</button
          >
          <button id="pause-btn" type="button" class="secondary" on:click={pauseMachine} disabled={!running}
            >Pausar</button
          >
        ...
        <dl class="state-grid">
          ...
          <div><dt>Cabezal</dt><dd id="head-index">{headIndex}</dd></div>
          <div><dt>Simbolo leido</dt><dd id="read-symbol">{readSymbol}</dd></div>
          <div><dt>Simbolo escrito</dt><dd id="written-symbol">{writtenSymbol}</dd></div>
    ...
    <aside class="side-panel" aria-labelledby="guide-title">
      <section class="card guide-card">
        ...
        <p>
          La aplicacion combina dos ideas: un codigo diario derivado de una fecha UTC y una maquina didactica
          que recorre el mensaje encriptado caracter por caracter para reconstruir el original.
        </p>
        ...
        <ul class="guide-list plain-list">
          ...
          <li>Ese valor se transforma con una formula fija para obtener un desplazamiento estable.</li>
          <li>
            El resultado final indica cuantas posiciones se mueve cada simbolo dentro del alfabeto del sistema.
          </li>
        </ul>
